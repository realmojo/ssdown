...
export function CreatePdfClient({ dict }: { dict?: any }) {
  ...
  const wrapText = (text: string, maxWidth: number, size: number, font: { widthOfTextAtSize: (t: string, s: number) => number }): string[] => {
    const lines: string[] = [];
    for (const paragraph of text.split("\n")) {
      if (paragraph.length === 0) { lines.push(""); continue; }
      let remaining = paragraph;
      while (remaining.length > 0) {
        if (font.widthOfTextAtSize(remaining, size) <= maxWidth) { lines.push(remaining); break; }
        let lo = 1, hi = remaining.length - 1, fit = 1;
        while (lo <= hi) {
          const mid = (lo + hi) >> 1;
          if (font.widthOfTextAtSize(remaining.slice(0, mid), size) <= maxWidth) { fit = mid; lo = mid + 1; } else { hi = mid - 1; }
        }
  ...
  const generatePdf = async () => {
    ...
    if (!hasContent) { setError("Please enter some text on at least one page."); return; }
    ...
    try {
      ...
      for (let i = 0; i < pages.length; i++) {
        ...
        let lineIdx = 0;
        while (lineIdx < allLines.length || (lineIdx === 0 && allLines.length === 0)) {
          const page = pdfDoc.addPage([pageWidth, pageHeight]);
          const chunk = allLines.slice(lineIdx, lineIdx + linesPerPage);
          for (let j = 0; j < chunk.length; j++) {
            page.drawText(chunk[j], { x: margin, y: pageHeight - margin - j * lineHeight, size: fontSize, font, color: rgb(0, 0, 0) });
          }
      ...
      setError("Failed to generate PDF. Please check your input and try again.");
    } finally {
        ...
        <div className="w-full max-w-2xl space-y-6">
          <div className="flex flex-col sm:flex-row gap-4">
            <div className="flex-1">
              ...
              <select value={pageSize} onChange={(e) => { setPageSize(e.target.value as PageSize); setResultUrl(null); }} className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2">
                {(Object.keys(PAGE_SIZE_LABELS) as PageSize[]).map((key) => (<option key={key} value={key}>{PAGE_SIZE_LABELS[key]}</option>))}
              </select>
            </div>
            <div className="w-full sm:w-40">
              <label className="block text-sm font-medium mb-2">Font Size: {fontSize}px</label>
              <input type="range" min={12} max={48} step={1} value={fontSize} onChange={(e) => { setFontSize(Number(e.target.value)); setResultUrl(null); }} className="w-full accent-red-600" />
              <div className="flex justify-between text-xs text-muted-foreground mt-1"><span>12</span><span>48</span></div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            {pages.map((page, idx) => (
              <div key={page.id} className="space-y-2 p-4 bg-muted/30 rounded-lg border border-muted">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <div className="flex items-center justify-center w-7 h-7 rounded-full bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 text-sm font-bold">{idx + 1}</div>
                    <span className="text-sm font-medium">Page {idx + 1}</span>
                  </div>
                  {pages.length > 1 && (<Button variant="ghost" size="icon" className="h-8 w-8 text-red-500 hover:text-red-700 hover:bg-red-100 dark:hover:bg-red-900/30" onClick={() => removePage(page.id)}><Trash2 className="w-4 h-4" /></Button>)}
                </div>
                <textarea value={page.text} onChange={(e) => updatePageText(page.id, e.target.value)} placeholder={`Enter text for page ${idx + 1}...`} rows={6} className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 resize-y min-h-[120px]" />
              </div>
            ))}
          </div>

          {error && (<div className="w-full p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg text-red-700 dark:text-red-300 text-sm">{error}</div>)}

          {!resultUrl && (
            <div className="flex justify-center pt-4">
              <Button size="lg" onClick={generatePdf} disabled={isProcessing} className="bg-red-600 hover:bg-red-700 text-white shadow-lg min-w-[200px]">
                {isProcessing ? (<><Loader2 className="w-5 h-5 mr-2 animate-spin" />Generating... {progress}%</>) : (<><FilePlus className="w-5 h-5 mr-2" />Generate PDF</>)}
              </Button>
            </div>
          )}
