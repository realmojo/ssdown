...
import { Download, Loader2, ArrowLeft, Heart, MessageCircle, Repeat, Eye, Calendar } from "lucide-react";
    ...
    setTimeout(() => {
      const mockData: TwitterResponse = {
        ...
        user: {
          ...
          avatar: "https://pbs.twimg.com/profile_images/1736600776930172929/f57UzfqX_normal.jpg"
        },
        content: "I’m happy with the verdict! Are you ? https://t.co/x93jQIIFVD",
        thumbnail: "https://pbs.twimg.com/amplify_video_thumb/1996749218937978880/img/-2MzuY9OU5_IK7IB.jpg",
        videoItems: [
          {
                    ...
                    <div className="p-6 flex flex-col md:flex-row gap-6 items-start">
                        ...
                        <div className="flex-1 w-full text-left space-y-6">
                            ...
                            <div className="flex items-center justify-between flex-wrap gap-2">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="min-w-0">
                                        <p className="font-bold text-sm text-gray-900 dark:text-gray-100 truncate">{data.user.name}</p>
                                        <p className="text-xs text-muted-foreground truncate">@{data.user.screenName}</p>
                                    </div>
                                </div>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-3 sm:grid-cols-5 gap-4 py-4 border-y border-gray-100 dark:border-gray-800">
                                    <div className="flex flex-col items-center gap-1 text-muted-foreground">
                                        ...
                                        <span className="text-xs font-medium">{formatNumber(data.stats.viewCount)}</span>
                                    </div>
                                    <div className="flex flex-col items-center gap-1 text-pink-500">
                                        <Heart className="w-4 h-4" />
                                        <span className="text-xs font-medium">{formatNumber(data.stats.favoriteCount)}</span>
                                    </div>
                                    <div className="flex flex-col items-center gap-1 text-blue-500">
                                        <MessageCircle className="w-4 h-4" />
                                        <span className="text-xs font-medium">{formatNumber(data.stats.replyCount)}</span>
                                    </div>
                                    <div className="flex flex-col items-center gap-1 text-green-500">
                                        <Repeat className="w-4 h-4" />
                                        <span className="text-xs font-medium">{formatNumber(data.stats.shareCount + data.stats.quoteCount)}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="space-y-3">
                                <p className="text-sm font-semibold text-muted-foreground uppercase tracking-wider">Download Options</p>
