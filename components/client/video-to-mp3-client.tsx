        ...
        {!file && (
          ...
          >
            ...
            <p className="text-lg font-medium mb-2">
              {dict?.video_to_mp3?.drop_zone || "Drag & drop a video file here, or click to browse"}
            </p>
            <p className="text-sm text-muted-foreground mb-1">
              {dict?.video_to_mp3?.supported || "Supported: MP4, WebM, MKV, AVI, MOV"}
            </p>
            <p className="text-xs text-muted-foreground">
              {dict?.video_to_mp3?.max_size || "Recommended max file size: 500MB"}
            </p>
          </div>
        )}
          ...
          >
            {loading ? (
              <>
                ...
                {dict?.video_to_mp3?.loading_ffmpeg || "Loading converter engine..."}
              </>
            ) : converting ? (
              <>
                <Loader2 className="mr-2 h-5 w-5 animate-spin" />
                {dict?.video_to_mp3?.converting || "Converting..."} {progress}%
              </>
            ) : (
              <>
      ...
      <div className="w-full max-w-6xl mx-auto mt-20 px-4 space-y-16">
        ...
        <section>
          <div className="text-center mb-12">
            ...
            <h2 className="text-3xl font-bold tracking-tight mb-4">
              {dict?.video_to_mp3?.guide_title || "How to Convert Video to MP3"}
            </h2>
            <p className="text-muted-foreground max-w-2xl mx-auto">
              {dict?.video_to_mp3?.guide_desc || "Follow these simple steps to extract audio from any video file."}
            </p>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {[
              {
                step: 1,
                title: dict?.video_to_mp3?.step1_title || "Select a Video File",
                desc: dict?.video_to_mp3?.step1_desc || "Drag and drop a video file or click to browse. Supports MP4, WebM, MKV, AVI, and MOV formats.",
                icon: Upload,
              },
              {
                step: 2,
                title: dict?.video_to_mp3?.step2_title || "Convert to MP3",
                desc: dict?.video_to_mp3?.step2_desc || "Click the Convert button. The conversion happens entirely in your browser using FFmpeg technology.",
                icon: Music,
              },
              {
                step: 3,
                title: dict?.video_to_mp3?.step3_title || "Download Your MP3",
                desc: dict?.video_to_mp3?.step3_desc || "Once conversion is complete, click Download to save the MP3 file to your device.",
                icon: Download,
              },
            ].map((step) => (
        ...
        <section className="bg-gray-50 dark:bg-gray-900/50 rounded-2xl p-8 md:p-12">
          <div className="text-center mb-10">
            ...
            <h2 className="text-3xl font-bold tracking-tight mb-4">
              {dict?.video_to_mp3?.tips_title || "Video to MP3 Conversion Tips"}
            </h2>
            <p className="text-muted-foreground">
              {dict?.video_to_mp3?.tips_desc || "Get the best results from your video to audio conversions."}
            </p>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {[
              {
                title: dict?.video_to_mp3?.tip1_title || "File Size Matters",
                desc: dict?.video_to_mp3?.tip1_desc || "Larger files take longer to convert. For best performance, use files under 500MB.",
                icon: FileVideo,
              },
              {
                title: dict?.video_to_mp3?.tip2_title || "100% Private",
                desc: dict?.video_to_mp3?.tip2_desc || "Your video never leaves your device. All processing happens locally in your browser.",
                icon: CheckCircle2,
              },
              {
                title: dict?.video_to_mp3?.tip3_title || "Audio Quality",
                desc: dict?.video_to_mp3?.tip3_desc || "The output MP3 is encoded at high quality (VBR ~190kbps) for the best balance of size and quality.",
                icon: Music,
              },
              {
                title: dict?.video_to_mp3?.tip4_title || "Browser Support",
                desc: dict?.video_to_mp3?.tip4_desc || "Works best in Chrome, Edge, and Firefox. Safari may have limited support for large files.",
                icon: Info,
              },
            ].map((tip, idx) => (
        ...
        <section>
          <div className="text-center mb-10">
            ...
            <p className="text-muted-foreground">
              {dict?.video_to_mp3?.features_desc || "Everything you need for video to audio conversion."}
            </p>
          </div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              {
                title: dict?.video_to_mp3?.feature1_title || "Browser-Based",
                desc: dict?.video_to_mp3?.feature1_desc || "No software installation needed. Convert directly in your web browser.",
              },
              {
                title: dict?.video_to_mp3?.feature2_title || "Multiple Formats",
                desc: dict?.video_to_mp3?.feature2_desc || "Supports MP4, WebM, MKV, AVI, and MOV video input formats.",
              },
              {
                title: dict?.video_to_mp3?.feature3_title || "High Quality Output",
                desc: dict?.video_to_mp3?.feature3_desc || "MP3 output with high-quality VBR encoding for excellent audio fidelity.",
              },
              {
                title: dict?.video_to_mp3?.feature4_title || "Zero Upload",
                desc: dict?.video_to_mp3?.feature4_desc || "Files are processed locally. Nothing is uploaded to any server.",
              },
            ].map((feature, idx) => (
        ...
        <section>
          <div className="text-center mb-10">
            ...
            <p className="text-muted-foreground">
              {dict?.qna_video_to_mp3?.desc || "Common questions about converting video to MP3 audio."}
            </p>
          </div>
