...
export const post: Post = {
  ...
  title: {
    en: "Copyright, Fair Use, and You: The Essential Guide to Safer Downloading",
    kr: "저작권과 공정 이용(Fair Use) 완전 정복: 안전한 다운로드를 위한 필수 법률 상식",
  },
  excerpt: {
    en: "Navigating the murky waters of digital copyright. Learn the difference between personal archiving and piracy, and how to use downloaded content ethically and legally.",
    kr: "디지털 저작권의 애매한 경계를 명확히 해드립니다. 개인 소장(사적 복제)과 불법 복제의 차이점, 그리고 다운로드한 콘텐츠를 윤리적이고 합법적으로 활용하는 방법을 알아보세요.",
  },
  content: {
    en: `
...
디지털 시대에 "저장(Save)" 버튼은 강력한 힘을 가집니다. **SSDown**과 같은 도구는 웹상의 콘텐츠를 여러분의 기기로 영구히 아카이빙할 수 있는 자유를 줍니다. 하지만 큰 힘에는 큰 책임이 따릅니다. "이 영상을 내 유튜브 에세이에 써도 될까?", "틱톡 챌린지를 저장하는 건 합법일까?", "도대체 공정 이용이 뭐지?"

이러한 개념을 오해하면 저작권 경고(Strike)를 받거나, 계정이 정지되거나, 심각하면 법적 분쟁에 휘말릴 수 있습니다. 이 가이드는 그 모호한 경계를 명확히 하기 위해 작성되었습니다. **법률 자문을 대체할 수는 없지만**, 미국 DMCA 및 한국 저작권법의 일반적인 원칙을 바탕으로 한 포괄적인 교육용 자료입니다.

---

## 1. 황금률: 사적 이용(개인 소장) vs 배포

저작권법에서 다운로드와 관련해 가장 중요한 구분선은 바로 **'배포(Distribution)'**입니다.

### 사적 이용을 위한 복제 (Private Copying)
한국 저작권법 제30조(사적 이용을 위한 복제)에 따르면, "공표된 저작물을 영리를 목적으로 하지 아니하고, 개인적으로 이용하거나 가정 및 이에 준하는 한정된 범위 안에서 이용하는 경우"에는 복제가 허용됩니다.
*   **허용됨**: 데이터가 잘 안 터지는 출근길에 보려고 넷플릭스나 유튜브 영상을 다운로드하는 행위.
*   **허용됨**: 친구에게 직접 보여주기 위해 웃긴 짤방을 폰에 저장하는 행위.

### 공중 송신 및 배포
여기서 99%의 문제가 발생합니다. 여러분은 타인의 저작물을 허락 없이 **대중에게 다시 공개할 권리가 없습니다.**
*   **금지됨**: 뮤직비디오를 다운받아 "내 채널"에 재업로드 (수익 창출 없어도 위법).
*   **금지됨**: 다운로드한 영화 클립을 오픈카톡방(불특정 다수)이나 커뮤니티 게시판에 올리는 행위.
*   **금지됨**: 유료 강의나 영상을 녹화해서 판매하는 행위 (심각한 범죄).

**요약**: 내 하드디스크에만 있으면? 대부분 괜찮습니다. 인터넷에 다시 올리면? 위험합니다.

---

## 2. '공정 이용(Fair Use)' 이해하기 (4가지 판단 기준)

공정 이용은 저작권자의 허락 없이도 저작물을 사용할 수 있는 예외적인 상황을 말합니다. 법원이 이를 판단할 때 주로 보는 4가지 요소가 있습니다.

### 1. 이용의 목적 및 성격
여러분의 사용이 **변형적(Transformative)**인가요? 새로운 의미나 비평을 추가했나요?
*   **공정 이용 가능성 높음**: 영화의 연기를 비판하기 위해 짧은 장면을 인용하고 내설을 덧붙인 리뷰 영상.
*   **침해 가능성 높음**: 단순히 조회수를 얻기 위해 남의 영상을 그대로 퍼 나르는 '렉카' 행위.

### 2. 저작물의 종류
*   사실적인 뉴스보다 창의적인 예술 작품(영화, 만화 등)의 저작권이 더 강하게 보호됩니다.

### 3. 이용된 분량과 비중
*   2시간짜리 영화에서 5초를 쓴 것은 공정 이용일 확률이 높습니다.
*   짧더라도 그 작품의 **핵심(Heart)** (예: 스포일러 결말, 가장 웃긴 하이라이트)을 사용했다면 침해로 간주될 수 있습니다.

### 4. 시장에 미치는 영향
여러분의 영상이 원작을 **대체**하나요?
*   사람들이 원본 대신 여러분의 영상을 봐서 원작자의 조회수/수익이 떨어진다면, 공정 이용으로 인정받기 어렵습니다.
*   반대로, 여러분의 리뷰 덕분에 원작 구매나 시청이 늘어난다면 긍정적으로 참작될 수 있습니다.
...
### 유튜브 (YouTube)
유튜브의 'Content ID' 로봇은 자동입니다. 여러분이 법적으로 공정 이용을 했다 해도, 로봇은 일단 차단하거나 수익을 가져갈 수 있습니다.
*   **팁**: 인용 목적으로 다운로드한 소스를 쓸 때는 7초 미만으로 짧게 끊어 쓰고, 내 해설 오디오가 주가 되게 하세요.
*   **팁**: 음원은 절대 피하세요. 3초만 써도 저작권 침해가 뜹니다.

### 틱톡 & 인스타그램 릴스
이곳은 '리믹스 문화'가 지배합니다. 춤을 따라 하거나 밈을 패러디하는 것은 권장되지만, 원본 영상을 그대로 가져와서 내 것인 양 올리는 것은 '불펌'입니다. 반드시 **원본 크리에이터를 태그(@)**하거나 멘션으로 출처를 밝히세요.

### 크리에이티브 커먼즈 (CC)
어떤 창작자들은 "출처만 밝히면 써도 좋다(CC BY)"라고 명시합니다. 이런 자료들을 적극 활용하세요.

---

## 4. 윤리적인 다운로더 체크리스트

SSDown에서 다운로드 버튼을 누르기 전에, 마음속으로 체크해보세요:

1.  **의도**: 나 혼자 즐기기 위함인가, 아니면 남의 콘텐츠로 관심을 끌기 위함인가?
2.  **출처**: 지인에게 공유할 때 "이거 00님이 만든 건데 쩔더라"라고 말했는가?
3.  **프라이버시**: 공개된 영상인가? 유출된 사생활 영상이나 해킹 자료는 절대 다운로드하거나 공유하지 마세요. (N번방 방지법, 디지털 성범죄 등 형사 처벌 대상입니다).

---

## 5. SSDown의 보호 정책

SSDown은 VCR이나 복사기 같은 도구입니다. 기술 자체는 중립적이지만, 사용 방식에 따라 결과가 달라집니다.
*   **콘텐츠 미보관**: 저희는 서버에 어떤 영상도 저장하지 않습니다. 여러분과 원본 사이트 간의 중계만 할 뿐입니다.
*   **불법 차단**: 명백한 불법 복제 사이트나 넷플릭스 등 DRM이 걸린 유료 콘텐츠의 다운로드는 기술적으로 차단하고 있습니다.

---

## 결론: 품격 있는 디지털 시민이 됩시다

저작권은 법 이전에 **존중(Respect)**의 문제입니다. 여러분이 즐겨 보는 그 영상은 누군가가 며칠 밤을 새워 만든 노력의 결실입니다. **SSDown**을 이용해 영감을 수집하고 즐거움을 소장하는 것은 환영합니다. 하지만 재배포할 때는 항상 자문해 보세요. "누군가 내 노력의 결과물을 이렇게 쓴다면 나는 기분이 좋을까?"

확신이 서지 않을 때는 허락을 구하세요. DM 하나 보내는 것으로 많은 문제를 예방하고, 때로는 새로운 인연을 만들 수도 있습니다.
    `,
  },
  category: "general",
