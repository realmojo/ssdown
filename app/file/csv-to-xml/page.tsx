...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const canonical = `${baseUrl}/file/csv-to-xml`;

  const title = dict.page_csv_to_xml.meta_title;
  const description = dict.page_csv_to_xml.meta_description;
...
export default async function CsvToXmlPage() {
  const locale = await getLocale();
  const dict = await getDictionary(locale);
  ...
  const faqSchema = {
    ...
    "@type": "FAQPage",
    mainEntity: dict.page_csv_to_xml.faq.map((item: { question: string; answer: string }) => ({
      "@type": "Question",
      name: item.question,
      acceptedAnswer: {
        "@type": "Answer",
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 4, name: dict.page_csv_to_xml.breadcrumb_title, item: "https://ssdown.app/file/csv-to-xml" },
    ],
  };
  ...
  const webAppSchema = {
    ...
    description: "Free online csv to xml tool. Fast, secure, and browser-based.",
  };

  const howToSchema = {
    "@context": "https://schema.org",
    "@type": "HowTo",
    name: "How to use Csv To Xml Online",
    description: "Use our free online csv to xml tool to process your files securely in your browser.",
    step: [
      {
        "@type": "HowToStep",
        position: 1,
